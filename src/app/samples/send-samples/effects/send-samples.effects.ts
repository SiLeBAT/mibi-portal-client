...
export class SendSamplesEffects {
    ...
    @Effect()
    commentDialogConfirm$: Observable<DisplayBanner | ValidateSamplesSuccess | SendSamplesSuccess> = this.actions$.pipe(
        ...
        map(([action, state]) => {
            ...
            const annotatedSampleSet: SampleSet = this.fromStoreToAnnotatedSampleSet(state);
            return from(this.dataService.sendSampleSheet({
                ...
                catchError((error) => {
                    ...
                    if (error instanceof InvalidInputError) {
                        this.store$.dispatch(new DisplayBanner({ predefined: 'validationErrors' }));
                        return of(new ValidateSamplesSuccess(error.data));
                    } else if (error instanceof InputChangedError) {
                        this.store$.dispatch(new DisplayBanner({ predefined: 'autocorrections' }));
                        return of(new ValidateSamplesSuccess(error.data));
                    } else if (error instanceof AuthorizationError) {
                        this.store$.dispatch(new LogoutUser());
                        return of(new DisplayBanner({ predefined: 'noAuthorizationOrActivation' }));
                    }
    ...
    private fromStoreToAnnotatedSampleSet(state: Samples): SampleSet {
        return {
